...
// and report{code,fightID}. `difficulty` is a QUERY-TIME filter, not an
// output field — WCL's rank objects carry no per-run difficulty label
// (confirmed live: every rank's `difficulty` key is null), so a raid caller
// that needs to know which difficulty was cleared must ask once per
// difficulty (3/4/5) and check whether any runs come back, rather than
// requesting once and reading a difficulty field off each result.
export const ENCOUNTER_RANKINGS = `
query EncounterRankings($name: String!, $serverSlug: String!, $serverRegion: String!, $encounterID: Int!, $metric: CharacterRankingMetricType, $byBracket: Boolean, $role: RoleType, $specName: String, $difficulty: Int) {
  characterData {
    character(name: $name, serverSlug: $serverSlug, serverRegion: $serverRegion) {
      encounterRankings(encounterID: $encounterID, metric: $metric, byBracket: $byBracket, role: $role, specName: $specName, difficulty: $difficulty)
    }
  }
}`;
