...
/**
 * LIVE Mythic+ zones of the current expansion, newest first — the same
 * "which zone id" concept `Character.wclZone` stores, distinct from the raid
 * zones above (a dungeon zone's difficulties list carries "Dungeon" instead
 * of "Mythic" — see the comment on RAID_ZONES in queries.js).
 */
export async function fetchDungeonZones({ refresh = false } = {}) {
  const data = await gql(RAID_ZONES, {}, { noCache: refresh });
  const expansions = data?.worldData?.expansions ?? [];
  if (!expansions.length) {
    dumpDebug('dungeon-zones-empty', { data });
    return [];
  }
  const current = [...expansions].sort((a, b) => b.id - a.id)[0];
  ...
  const zones = (current.zones ?? []).filter((z) => {
    ...
    if (NOT_A_REAL_RAID.test(z.name ?? '')) return false;
    const diffs = (z.difficulties ?? []).map((d) => d.name);
    if (!diffs.includes('Dungeon')) return false;
