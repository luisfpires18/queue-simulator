...
export function buildReport(bundle) {
  ...
  const runConfidence = assessConfidence({
    ...
    missingData: detectMissingData(mineDetail, otherDetail, mine, them, buffSources),
  });
  for (const g of gaps) {
    g.advice = adviceFor(g);
    g.priority = priorityOf(g.severity);
    g.confidence = gapConfidence(runConfidence, sampleInfoFor(g));
  }
  ...
  return {
    headline,
    // 0 — the top-of-report summary, derived from the same gaps below
    summary: buildSummary({ headline, gaps, confidence: runConfidence }),
    // 1 — the picker
 ...
 */
export function buildGaps(mineDetail, otherDetail, buffSources = {}, { truncated = false } = {}) {
  ...
  for (const g of gaps) {
    g.advice = adviceFor(g);
    g.priority = priorityOf(g.severity);
    g.confidence = gapConfidence(confidence, sampleInfoFor(g));
  }
  return gaps;
}

/** Run-level confidence, standalone — for callers (raid.js) that don't already
 * have the `mine`/`them` metrics objects buildGaps() computes internally. */
export function assessRunConfidence(mineDetail, otherDetail, buffSources = {}, { truncated = false } = {}) {
  const mine = computeRunMetrics(mineDetail);
  const them = computeRunMetrics(otherDetail);
...
/** What's missing from either side's log that would make a comparison shakier. */
function detectMissingData(mineDetail, otherDetail, mine, them, buffSources) {
  const missing = [];
  if (!mineDetail.deaths || !otherDetail.deaths) missing.push('death data');
  if (mine.downtime.idlePct == null || them.downtime.idlePct == null) missing.push('cast-timing data');
  if (!mine.resource && !them.resource) missing.push('resource data');
  if (!buffSources || !Object.keys(buffSources).length) missing.push('buff-source attribution');
  return missing;
}

/** A gap whose own sample (total presses observed) is thin gets its confidence
 * stepped down, even when the run-level comparison is otherwise solid. */
function sampleInfoFor(g) {
...
function gapsFrom(mine, them, buffSources, isKill = true) {
  ...
  // Which ability names the cpm gap already names below, so the per-ability loop
  // doesn't repeat the same root cause as a second, separate bullet.
  let cpmBehindNames = new Set();

  if (them.engagedCPM && mine.engagedCPM < them.engagedCPM * 0.97) {
    const diffPct = (100 * (them.engagedCPM - mine.engagedCPM)) / them.engagedCPM;
    // "you cast less" is useless on its own — say WHICH buttons lagged, and by how
    // much your RATE trailed theirs (measured over active-combat time), not raw
    // cast counts — a raw-count diff reads like "you missed N casts" even when the
    // two fights simply ran different lengths.
    const behind = diffs
      .filter((d) => d.theirCpm - d.myCpm >= 0.5)
      .sort((a, b) => (b.theirCpm - b.myCpm) - (a.theirCpm - a.myCpm))
      .slice(0, 4)
      .map((d) => ({ name: d.name, myCpm: round1(d.myCpm), theirCpm: round1(d.theirCpm), cpmBehindBy: round1(d.theirCpm - d.myCpm) }));
    cpmBehindNames = new Set(behind.map((b) => b.name));
    gaps.push(
      gap('cpm', 'Casts per minute (active time)', round1(mine.engagedCPM), round1(them.engagedCPM), 'CPM', diffPct * 0.8, { behind })
    );
  }

  for (const row of diffs) {
    if (row.severity < 0.5) continue;
    if (cpmBehindNames.has(row.name)) continue; // already named in the cpm gap's behind list — same root cause
    gaps.push(
...
function abilityDiffs(mine, them) {
  ...
  for (const name of names) {
    ...
    // Engaged-time rate, not full-duration — so a downtime-heavy fight doesn't
    // masquerade as a rotation gap (that's already its own 'downtime' gap above).
    const myCpm = mine.abilities.get(name)?.engagedCpm ?? 0;
    const theirCpm = them.abilities.get(name)?.engagedCpm ?? 0;
