...
// right one. All combat potions share one 5-minute cooldown.
//
...
// Season 1 (Midnight) Mythic+ dungeon timers, in seconds — keyed by the exact
// dungeon name WCL reports as `fight.name`. The ceiling on potion uses is the
// dungeon's fixed timer, not how long THIS particular pull happened to take —
// a fast clear and a barely-timed run are judged against the same number, so a
// quick clear doesn't get told it "wasted" potions it was never going to need.
// Verified against two known-correct values: Skyreach (28:00 -> 5 max) and
// Seat of the Triumvirate (34:00 -> 6 max). Season-specific — will go stale
// when the dungeon pool rotates, same as the spec icon table in icons.js; a
// dungeon not in this table falls back to the run's own duration below.
const DUNGEON_TIMER_SEC = {
  'Windrunner Spire': 33 * 60 + 30,
  'Maisara Caverns': 33 * 60,
  "Magister's Terrace": 33 * 60,
  'Nexus-Point Xenas': 29 * 60 + 30,
  "Algeth'ar Academy": 29 * 60 + 30,
...
/**
 * Potions drunk, and how many the run allowed.
 *
...
function potionsOf(detail) {
  ...
  const timerSec = DUNGEON_TIMER_SEC[detail?.fight?.name];
  const max =
    timerSec != null
      ? Math.floor(timerSec / POTION_CD_SEC)
      : (() => {
          // Not a known M+ dungeon (a raid boss, or a dungeon not yet in the
          // table above) — fall back to this run's own duration, with the one
          // free pre-pot a fixed-timer count already accounts for implicitly.
          const sec = fightSec(detail);
          return sec > 0 ? 1 + Math.floor(sec / POTION_CD_SEC) : null;
        })();
...
export function buildConsumables(mineDetail, otherDetail, otherName, buffSources = null, statPriorityNote = null) {
  ...
  if (potions.mine.missed > 0) {
    notes.push(
      `You drank ${potions.mine.used} potion${potions.mine.used === 1 ? '' : 's'} but this run allowed up to ${potions.mine.max} ` +
        `(they share a 5-minute cooldown). That's ${potions.mine.missed} free burst you left unused.`
    );
  }
