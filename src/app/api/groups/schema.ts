import { z } from "zod";
import { isStartInPast, isStartTooFarOut } from "@/game/scheduling";
...
const baseFields = {
  ...
  // The server-side backstop for a direct API call bypassing the picker
  // (which already restricts itself to later today - see useTodayTimeRange in
  // GroupFormShared.tsx). Same shared rule the form applies, so the two can't
  // drift apart.
  startsAt: z
    .string()
    .datetime()
    .nullish()
    .refine((val) => !isStartInPast(val), { message: "Start time can't be in the past." })
    .refine((val) => !isStartTooFarOut(val), { message: "Start time must be today." }),
  slots: slotsSchema,
