...
import { buildAbilityTable, buildGaps, assessRunConfidence } from '../analysis/compare.js';
import { buildSummary } from '../analysis/reportSummary.js';
...
export async function buildRaidPull({
  ...
  // A wipe compared against a truncated (fair-window) slice of the benchmark's
  // kill is inherently less certain than a kill-vs-kill comparison — feed that
  // into confidence rather than presenting both cases identically.
  const truncated = cutoffSec != null;
  const gaps = buildGaps(mineDetail, otherDetail, buffSources, { truncated });
  const runConfidence = assessRunConfidence(mineDetail, otherDetail, buffSources, { truncated });
  ...
  return {
    ...
    headline,
    // the top-of-report summary, derived from the same gaps below
...
export async function buildRaidReport({
  ...
  // Reject the wrong log at the menu step — but only on NAME+CLASS, not spec: a
  // report can span a respec (this same character logged on two specs across
  // different bosses), so a fixed roster assumption here would falsely reject
  // every boss but the one fights[0] happened to catch them on.
  const anyFight = report.fights[0];
  if (anyFight) {
    await assertCharacterInLog({ code, fightID: anyFight.id, name, className, specName: null, classLabel, refresh });
  }
  ...
  // NOW check the spec — scoped to THIS boss's own fights, since a respec means
  // the roster genuinely differs boss-to-boss within the same report.
  await assertCharacterInLog({ code, fightID: attempts[0].id, name, className, specName, classLabel, refresh });
