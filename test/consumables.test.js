...
import { buildConsumables } from '../src/server/analysis/consumables.js';
...
// A known M+ dungeon's potion ceiling comes from its own fixed keystone timer,
// not from how long this particular pull took — confirmed against two known
// values (a fast Skyreach clear still tops out at 5, not 6; a longer Seat of
// the Triumvirate run tops out at 6, not 7).
test('potions: a known dungeon caps at its own timer, regardless of this pull\'s actual duration', () => {
  // actual pull was only 10 minutes, but Skyreach's 28:00 timer still caps at 5
  const fast = { fight: { name: 'Skyreach', startTime: 0, endTime: 600000 }, buffs: { totalTimeMs: 600000, auras: [] } };
  const fastC = buildConsumables(fast, fast, 'X');
  assert.equal(fastC.potions.mine.max, 5);

  const seat = { fight: { name: 'Seat of the Triumvirate', startTime: 0, endTime: 600000 }, buffs: { totalTimeMs: 600000, auras: [] } };
  const seatC = buildConsumables(seat, seat, 'X');
  assert.equal(seatC.potions.mine.max, 6);
});

test('potions: an unrecognised dungeon (or a raid boss) falls back to this run\'s own duration', () => {
  // no fight.name match in the timer table -> old behaviour: 1 + floor(sec/300)
  const d = { fight: { name: 'Some Raid Boss', startTime: 0, endTime: 660000 }, buffs: { totalTimeMs: 660000, auras: [] } };
  const c = buildConsumables(d, d, 'X');
  assert.equal(c.potions.mine.max, 3); // 1 + floor(660/300)
});
