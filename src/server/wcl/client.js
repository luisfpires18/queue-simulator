...
export async function gql(query, variables = {}, opts = {}) {
  ...
  if (!res.ok) {
    ...
    // WCL's own outage/rate-limit pages come back as an HTML error page, not
    // JSON — dumping that raw markup into the UI (surfaced verbatim via
    // errorResponse() in wclHelpers.ts) is unreadable. Log the full body for
    // diagnostics, but only forward a real message to the user when WCL gave
    // us one as structured JSON.
    let detail = '';
    try {
      const parsed = JSON.parse(bodyText);
      detail = parsed.error || parsed.message || '';
    } catch {
      // not JSON — almost certainly WCL's generic HTML error page
    }
    console.error(`[wcl] API HTTP ${res.status}${detail ? `: ${detail}` : ''}`, bodyText.slice(0, 2000));
    throw new Error(
      detail
        ? `Warcraft Logs API error (HTTP ${res.status}): ${detail}`
        : `Warcraft Logs is temporarily unavailable (HTTP ${res.status}). Try again in a bit.`
    );
  }
