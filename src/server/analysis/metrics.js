...
export const IGNORED_ABILITIES = new Set([
  ...
  'Void-Touched', // a consumable (rune) proc, not a rotation/gameplay decision
]);
...
export function computeRunMetrics(detail) {
  ...
  // include death → re-engage stretches). Rates measured only inside these
  ...
  const perEngagedMinute = (count) => (engagedMinutes > 0 ? count / engagedMinutes : 0);

  const abilities = new Map();
  for (const a of detail.casts?.abilities ?? []) {
    abilities.set(a.name, { casts: a.casts, cpm: perMinute(a.casts), engagedCpm: perEngagedMinute(a.casts) });
  }
  ...
  return {
    ...
    engagedCPM: perEngagedMinute(totalCasts),
    abilities,
    auras,
    damageShare,
    deaths,
